...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript. Claude Code keeps each session at `${CLAUDE_CONFIG_DIR:-~/.claude}/projects/<slug>/<session-id>.jsonl`, where `<slug>` is the workspace path with every `/` turned into `-` (so `/Users/you/proj` becomes `-Users-you-proj`), and its subagents under `<session-id>/subagents/`. Codex keeps sessions at `~/.codex/sessions/<yyyy>/<mm>/<dd>/rollout-*.jsonl`. Every line is one event. Read only the current workspace's directory. Globbing across projects reads unrelated private chats. Walk the log against what actually happened:
...
Before handing back, spawn a reviewer on a different model family from the one that did the work, such as `codex exec` from Claude Code. If none is available, spawn a fresh subagent that saw none of the work. Self-review in the same context is not a substitute. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
